...
> **Current Phase:** Figma implementation + screen/backend mapping
...
| 3     | Booking system, payments (Paystack), subscriptions, reports generator                                                           | Week 11–13 | ✅ Complete |
| 4     | School dashboard, AI features, teletherapy, multi-language, mobile app                                                          | Week 14–18 | 🔴 Pending |

---

## Figma Screen + Backend Mapping

| Figma screen | Frontend route | Backend integration |
| ------------ | -------------- | ------------------- |
| Home landing | `/` | Public route, design assets in `frontend/public/design-assets` |
| Parent login / therapist login | `/login` | `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout` |
| Parent signup | `/register?role=PARENT` | `POST /api/v1/auth/register` with role `PARENT` |
| Therapist signup | `/register?role=THERAPIST` | `POST /api/v1/auth/register` with role `THERAPIST` + `areaofexpertise` |
| Administrator signup | `/register/admin` | Admin registration flow and invite-code guarded backend validator |
| Verify account | `/verify-otp` | `POST /api/v1/auth/send-otp`, `/verify-otp`, `/resend-otp` |
| Parent dashboard | `/dashboard` | `GET /api/v1/children`, Redux auth + child slices |
| Therapist dashboard | `/dashboard` | `GET /api/v1/children`, therapist-assigned child view |
| Admin overview | `/dashboard` for `ADMIN` | Dashboard metrics cards and chart scaffolding |
| Admin user database | `/admin` | `GET /api/v1/admin/users`, `GET /api/v1/admin/stats`, `PATCH /api/v1/admin/users/:id/approve` |
| Children database | `/children` | `GET/POST/PATCH/DELETE /api/v1/children` |
| Child profile + intake | `/children/[id]`, `/children/[id]/intake` | `GET /api/v1/children/:id`, `GET/PUT /api/v1/intake/:childId` |
| Session notes | `/children/[id]/sessions`, `/children/[id]/sessions/new` | `GET/POST/PATCH/DELETE /api/v1/sessions`, `PUT /api/v1/sessions/:id/notes` |
| Progress dashboard | `/progress/[childId]` | `GET /api/v1/progress/:childId` |
| Messages | `/messages`, `/messages/[id]` | `GET/POST /api/v1/messages/conversations`, `GET/POST /api/v1/messages/conversations/:id/messages`, Socket.io events |
| Content management | `/resources`, `/resources/new` | `GET/POST/PUT/DELETE /api/v1/resources` |
| Therapist accounts | `/therapists`, `/therapists/[id]` | `GET /api/v1/therapists`, `GET /api/v1/therapists/:id` |
| Availability | `/availability` | `GET/POST/PUT/DELETE /api/v1/availability` |
| Bookings | `/bookings`, `/bookings/new`, `/bookings/[id]/payment` | `GET/POST /api/v1/bookings`, `PATCH /api/v1/bookings/:id/status`, Paystack payment init/verify |
| Subscriptions | `/subscriptions` | `GET /api/v1/subscriptions/plans`, `GET /api/v1/subscriptions/my`, `POST /api/v1/subscriptions/subscribe` |
| Reports | `/reports` | `GET /api/v1/reports/:childId` PDF download |
| Notifications | `/notifications` | `GET /api/v1/notifications`, `PATCH /api/v1/notifications/:id/read`, `PATCH /api/v1/notifications/read-all` |
| Admin revenue | `/admin/revenue` | `GET /api/v1/payments/revenue`, `GET /api/v1/payments/transactions` |

## Demo Login Credentials

| Role | Email | Password | Seed status |
| ---- | ----- | -------- | ----------- |
| Admin | `admin@neurobridge.com` | `Admin@123` | Existing seeded admin |
| Parent | `parent@neurobridge.com` | `Parent@123` | Added to seed script |
| Therapist | `therapist@neurobridge.com` | `Therapist@123` | Added to seed script, approved |
| Pending therapist sample | `lisa@example.com` | `Therapist@123` | Existing seeded pending approval account |
...
| 2026-08-20 | Figma Design System Implementation              | Applied Plus Jakarta Sans, Figma color tokens, auth shell, landing, dashboard panels, tables, badges, and shared states |
| 2026-08-20 | Screen/Backend Integration Map                  | Added route-to-endpoint mapping for auth, admin, children, sessions, resources, messages, bookings, subscriptions, reports, and notifications |
| 2026-08-20 | Demo Credentials                                | Added deterministic demo parent and therapist accounts to `backend/prisma/seed.js`; documented admin/parent/therapist credentials |
